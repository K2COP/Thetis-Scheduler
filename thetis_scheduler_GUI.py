...
def apply_schedule(freq_hz, mode, time_str, days):
    """Apply a schedule by setting frequency and mode."""
    logging.info(f"Applying schedule: {freq_hz} Hz, {mode}, {time_str}, {days}")
    success = set_frequency_and_mode(freq_hz, mode)
    if not success:
        logging.error("Failed to apply schedule")
    return success

def create_schedule(freq, mode, time_str, days):
    """Create a schedule for frequency and mode change."""
    try:
        ...
        if days == "Daily":
            schedule.every().day.at(time_str).do(apply_schedule, freq_hz, mode, time_str, days)
        elif days == "Monday thru Friday":
            for day in ["monday", "tuesday", "wednesday", "thursday", "friday"]:
                getattr(schedule.every(), day).at(time_str).do(apply_schedule, freq_hz, mode, time_str, days)
        else:
            day = days.lower()
        ...
        logging.info(f"Scheduled: {freq} MHz, {mode}, {time_str}, {days}")
    except Exception as e:
...
def add_schedule():
    try:
        ...
        schedules.append({"freq": freq, "mode": mode, "time": time_str, "days": days})
        save_schedules(schedules)
        
        # Clear and reload all schedules
        schedule.clear()
        for s in schedules:
            create_schedule(s["freq"], s["mode"], s["time"], s["days"])
...
def update_schedules_listbox():
    ...
    try:
        ...
        for schedule in schedules:
            ...
            days = schedule["days"]
            display_str = f"{freq} MHz, {mode}, {time_str}, {days}"
